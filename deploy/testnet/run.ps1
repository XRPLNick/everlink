...
$net = $env:EV_NETWORK
$stage = if ($env:NOMAD_STAGE) { $env:NOMAD_STAGE } else { "deploy" }   # hosts | deploy | demo
...
if (-not (Test-Path $tenantFile)) {
  if ($net -eq "mainnet") { Fail "no-tenant" "create deploy\testnet\tenant.mainnet.json yourself (see tenant.mainnet.example.json) — real funds are never generated by these scripts" }
