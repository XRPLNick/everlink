#!/usr/bin/env node
'use strict';
// Writes contract/dist/nomad.config.json for this tenant and network from the template in
// deploy/testnet/nomad.config.testnet.json (Windows PowerShell 5.1's ConvertTo-Json is not
// reliable enough for this: it threw OutOfMemoryException on the same object).
//   node deploy/testnet/patch-config.js        (EV_NETWORK, NOMAD_SIZE)
const fs = require('fs');
const path = require('path');

const NETWORK = (process.env.EV_NETWORK || 'testnet').toLowerCase();
const SIZE = Number(process.env.NOMAD_SIZE || 3);
const root = path.join(__dirname, '..', '..');
const cfgPath = path.join(root, 'contract', 'dist', 'nomad.config.json');
const tenant = JSON.parse(fs.readFileSync(path.join(__dirname, `tenant.${NETWORK}.json`), 'utf8'));
const cfg = JSON.parse(fs.readFileSync(path.join(__dirname, 'nomad.config.testnet.json'), 'utf8'));
